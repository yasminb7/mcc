...
maxvallength = max( [len(i[0]) for i in doc] )
maxdoclength = max( [len(i[1]) for i in doc] )

print "".join(maxvallength*["="]), "\t", "".join(maxdoclength*["="])
for name, docstr in doc:
    if docstr=="":
        print docstr, "".join((maxvallength-len(docstr))*[" "]), "\t", docstr
        print name, "".join((maxvallength-len(name))*[" "]), "\t", docstr
    else:
        print "`%s`" % name, "".join((maxvallength-len(name))*[" "]), "\t", docstr
print "".join(maxvallength*["="]), "\t", "".join(maxdoclength*["="])
